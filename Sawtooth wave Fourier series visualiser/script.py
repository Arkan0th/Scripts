...
for n, term in terms:
    plt.plot(x, term, linestyle=':', label=f'člen {n}: $\\frac{{2(-1)^{{{n+1}}}}}{{{n}}} \\sin({n}x)$')
plt.plot(x, f_approx, label=f'Suma {k} členů', color='blue', linestyle='--', linewidth=2)
...
plt.xlabel('x')
plt.ylabel('f(x)')
plt.title(f'Fourierova řada pro f(x) = x, k = {k}')
plt.axhline(0, color='gray', linewidth=0.5)
plt.axvline(0, color='gray', linewidth=0.5)
plt.grid(True)
plt.legend(loc='upper left')
plt.tight_layout()
plt.savefig("Fourier_plot.png", dpi=300)
plt.show()
