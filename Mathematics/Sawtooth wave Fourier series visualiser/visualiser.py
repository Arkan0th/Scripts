...
def plot_fourier_series(k, points=1000, save_path=None):
    ...
    plt.title(f'Fourier series of f(x) = x, k = {k}', fontsize=26)
    plt.grid(True)
...
def generate_fourier_animation(max_k=50, step=1, save_dir="frames"):
    os.makedirs(save_dir, exist_ok=True)
    total_steps = (max_k - 1) // step + 1
    
    for i, k in enumerate(range(1, max_k + 1, step), start=1):
        save_path = os.path.join(save_dir, f"frame_k{k:03d}.jpg")
